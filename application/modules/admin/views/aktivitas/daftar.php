<div class="page page-tables-footable" style="z-index:0">
					...
					<div class="table-responsive">
						<table id="searchTextResults" data-filter="#filter" data-page-size="5" class="footable table table-custom footable-loaded phone breakpoint">
							...
							<tbody>
								...
								$get_event = $this->db->query("SELECT * FROM content WHERE category='aktivitas'");
								...
								foreach ($hasil_event as $item){
									...
									<tr>
										...
										<td>
											...
											<a href="<?php echo base_url().'web/read/aktivitas/'.$item->id?>" target="_blank" title="Preview" class="btn btn-info btn-rounded btn-icon-only btn-ef btn-ef-7 btn-ef-7c mb-10" style="position:relative;padding:10px 15px;text-decoration:none">
												<i class="fa fa-file-text-o" style="font-size:15px;position:relative;line-height:10px"></i>
											</a>
											<a href="<?php echo base_url().'admin/aktivitas/ubah/'.$item->id?>" title="Ubah" class="btn btn-warning btn-rounded btn-icon-only btn-ef btn-ef-7 btn-ef-7c mb-10" style="position:relative;padding:10px 15px">
	...
	<script>
		...
		function hapus(){
			...
			$.ajax( {  
				type :"post",  
				url :"<?php echo base_url() . 'admin/aktivitas/hapus' ?>",  
				cache :false,  
				data :{id:id},
				success : function(data) { 
					...
					setTimeout(function(){window.location = "<?php echo base_url() . 'admin/aktivitas/daftar' ?>";},1000);              
				},  
				error : function() {  
